...
function targetCount(resources) {
  const targets = new Set();
  resources.forEach((item) => splitList(item.targets).forEach((target) => targets.add(target)));
  return targets.size;
}
  ...
  return `
    <article class="radar-lounge-row">
      <div>
        ...
        <div class="radar-lounge-chipline">${reasons.length ? reasons.map((reason) => `<i>${escapeHtml(reason)}</i>`).join("") : `<i>확인 완료</i>`}</div>
      </div>
...
function focusResource(id, title) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => card?.classList.remove("radar-highlight"), 1600);
  }, 140);
}

function bindEvents(home, resources) {
  home.querySelectorAll("[data-lounge-view]").forEach((button) => button.addEventListener("click", () => navClick(button.dataset.loungeView)));
  home.querySelectorAll("[data-lounge-filter='review']").forEach((button) => button.addEventListener("click", () => {
    const status = $("#resourceStatus");
    if (status) status.value = "검토 필요";
    navClick("resources");
  }));
  home.querySelectorAll("[data-lounge-focus]").forEach((button) => button.addEventListener("click", () => focusResource(button.dataset.loungeFocus, button.dataset.title)));
  home.querySelectorAll("[data-lounge-share]").forEach((button) => button.addEventListener("click", () => {
    const item = resources.find((resource) => resource.id === button.dataset.loungeShare);
    if (item) copyText(buildShareText(item));
  }));
  home.querySelectorAll("[data-lounge-copy='brief']").forEach((button) => button.addEventListener("click", () => copyText(buildBrief(resources))));
}
...
function renderLounge() {
  ...
  const lastAuto = localStorage.getItem(AUTO_KEY) || "아직 없음";
  ...
  home.innerHTML = `
    <div class="radar-lounge-hero">
      ...
      <aside class="radar-lounge-side">
        ...
        <div class="radar-lounge-priority">
          ${priority.length ? priority.slice(0, 3).map((item) => `<article><b>${escapeHtml(item.title || "제목 미정")}</b><span>${escapeHtml(item.agency || "기관 미기재")} · ${escapeHtml(dueText(item.deadline))} · ${escapeHtml(reviewReasons(item).slice(0, 2).join(", ") || "확인 완료")}</span></article>`).join("") : `<article><b>등록된 자원이 없습니다</b><span>샘플 자원을 불러오거나 자료를 수집해보세요.</span></article>`}
        </div>
      </aside>
    </div>
    <div class="radar-lounge-kpis">
      ...
      ${kpi("대상자 유형", targetCount(resources), "자원에 등록된 대상 조건")}
    </div>
    <div class="radar-lounge-workgrid">
      <section class="radar-lounge-card">
        <div class="radar-lounge-head"><div><strong>오늘의 검토함</strong><span>우선순위가 높은 자원부터 확인합니다.</span></div><button class="ghostish" type="button" data-lounge-filter="review">전체 보기</button></div>
        <div class="radar-lounge-list">${priority.length ? priority.map((item) => resourceRow(item)).join("") : emptyRow("아직 검토할 자원이 없습니다.")}</div>
      </section>
      <div class="radar-lounge-stack">
        <section class="radar-lounge-card">
          <div class="radar-lounge-head"><div><strong>빠른 작업</strong><span>자주 쓰는 동작만 모았습니다.</span></div></div>
          <div class="radar-lounge-quick"><button type="button" data-lounge-view="collect">자원 수집</button><button type="button" data-lounge-view="resources">검토함</button><button type="button" data-lounge-view="important">중요 자원</button><button type="button" data-lounge-copy="brief">요약 복사</button></div>
        </section>
        <section class="radar-lounge-card">
          <div class="radar-lounge-head"><div><strong>수집 상태</strong><span>자동 수집이 조용히 실패하지 않게 보는 영역입니다.</span></div></div>
          <div class="radar-lounge-source-log"><div><span>마지막 자동 수집</span><em>${escapeHtml(lastAuto)}</em></div><div><span>마감 임박</span><em>${dueSoon.length}건</em></div><div><span>검토 대기</span><em>${review.length}건</em></div></div>
        </section>
      </div>
    </div>
  `;
  bindEvents(home, resources);
}

function scheduleRender() {
  clearTimeout(scheduleRender.timer);
  scheduleRender.timer = setTimeout(renderLounge, 120);
}
