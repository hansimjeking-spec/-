...
function extractDocuments(item) {
  ...
  return match ? match[0].replace(/\s+/g, " ").trim() : "제출서류 확인 필요";
}
...
function mountDashboardEnhancements() {
  ...
  const queue = [...resources]
    ...
    .slice(0, 6);
  const duplicates = scanDuplicates(resources);
  const signature = JSON.stringify({
    total: resources.length,
    queue: queue.map((entry) => [entry.item.id, entry.item.updatedAt, entry.reasons.join("|")]),
    duplicates: duplicates.length
  });

  let hub = $("#radarWorkflowHub");
  if (hub && signature === lastDashboardSignature) return;
  ...
  hub.innerHTML = `
    ...
    ${resources.length ? "" : `
      <div class="radar-empty-guide">
        <div><strong>처음 사용이라면 샘플을 먼저 불러오세요.</strong><p>샘플 자원으로 대시보드, 대상자 추천, 브리핑 흐름을 바로 확인할 수 있습니다.</p></div>
        <div class="button-row"><button class="primary-button" id="radarLoadSamples" type="button">샘플 자원 불러오기</button><button class="ghost-button" data-radar-open="collect" type="button">직접 수집하기</button></div>
      </div>`}
    <div class="radar-queue-panel">
      <div class="radar-panel-title"><div><strong>오늘 검토할 자원</strong><span>마감 임박, 고긴급, 누락 항목을 먼저 보여줍니다.</span></div><em>중복 의심 ${duplicates.length}건</em></div>
      ${queue.length ? queue.map(({ item, reasons }) => `
        <article class="radar-queue-item">
          <div><strong>${escapeHtml(item.title || "제목 미정")}</strong><span>${escapeHtml(item.agency || "기관 미기재")} · ${escapeHtml(item.deadline || "마감 미정")} · ${escapeHtml(daysLeftText(item.deadline))}</span><p>${reasons.slice(0, 4).map((reason) => `<i>${escapeHtml(reason)}</i>`).join("")}</p></div>
          <button class="ghost-button" data-radar-focus="${escapeHtml(item.id)}" type="button">목록에서 보기</button>
        </article>`).join("") : `<div class="radar-soft-empty">오늘 우선 검토할 자원이 없습니다. 새 공고를 수집하면 이곳에 정리됩니다.</div>`}
    </div>
  `;

  hub.querySelectorAll("[data-radar-open]").forEach((button) => button.addEventListener("click", () => activeView(button.dataset.radarOpen)));
  ...
  hub.querySelectorAll("[data-radar-focus]").forEach((button) => button.addEventListener("click", () => focusResourceById(button.dataset.radarFocus)));
}
...
function mountCollectGuides() {
  ...
  if (panel && !$("#radarCollectionGuide")) {
    ...
    if (help) help.innerHTML = "복지로 자동 수집을 사용하려면 공공데이터포털 인증키가 필요합니다. 관리자에게 <code>DATA_GO_KR_SERVICE_KEY</code> 등록을 요청하세요.";
    panel.insertAdjacentHTML("beforeend", `<div id="radarCollectionGuide" class="radar-collection-guide"><strong>수집 성공/실패 확인표</strong><div class="radar-source-table">${SOURCE_GUIDES.map(([name, description, action]) => `<div><b>${escapeHtml(name)}</b><span>${escapeHtml(description)}</span><em>${escapeHtml(action)}</em></div>`).join("")}</div></div>`);
  }

  const status = $("#documentStatus");
  if (status && !$("#radarDocumentGuide")) {
    status.insertAdjacentHTML("afterend", `<div id="radarDocumentGuide" class="radar-document-guide"><strong>문서가 안 읽힐 때 확인</strong><ul><li>배포용 한글 문서는 HWPX로 다시 저장한 뒤 업로드합니다.</li><li>스캔 PDF는 텍스트 선택이 되지 않으므로 OCR 변환이 필요합니다.</li><li>암호가 걸린 파일은 암호 해제 후 다시 올립니다.</li><li>표가 이미지로 들어간 공문은 텍스트를 복사해 붙여넣는 편이 빠릅니다.</li></ul></div>`);
  }
}
...
function mountReviewGuide() {
  ...
  panel.querySelector(".panel-head")?.insertAdjacentHTML("afterend", `<div id="radarReviewGuide" class="radar-review-guide"><strong>저장 전 필수 확인</strong><span>마감일</span><span>대상 조건</span><span>신청방법</span><span>문의처</span><span>제출서류</span><span>원문 근거</span></div>`);
}
...
function mountDuplicatePanel() {
  ...
  const signature = duplicates.map(([a, b]) => `${a.id}:${b.id}`).join("|");
  let panel = $("#radarDuplicatePanel");
  if (panel && signature === lastDuplicateSignature) return;
  ...
  panel.innerHTML = duplicates.length ? `<div class="radar-panel-title"><div><strong>중복 의심 자원</strong><span>제목·기관·마감일이 비슷한 항목입니다. 병합 전 원문을 확인하세요.</span></div><button class="ghost-button" id="copyDuplicateReport" type="button">중복 목록 복사</button></div>${duplicates.map(([a, b]) => `<article class="radar-duplicate-item"><div><strong>${escapeHtml(a.title)}</strong><span>${escapeHtml(a.agency || "기관 미기재")} · ${escapeHtml(a.deadline || "마감 미정")}</span></div><div><strong>${escapeHtml(b.title)}</strong><span>${escapeHtml(b.agency || "기관 미기재")} · ${escapeHtml(b.deadline || "마감 미정")}</span></div><button class="text-button" data-radar-focus="${escapeHtml(a.id)}" type="button">첫 항목 보기</button></article>`).join("")}` : `<div class="radar-soft-empty">중복으로 보이는 자원이 없습니다.</div>`;
  panel.querySelector("#copyDuplicateReport")?.addEventListener("click", () => copyText(duplicates.map(([a, b], index) => `${index + 1}. ${a.title} / ${b.title}`).join("\n") || "중복 의심 자원이 없습니다.", "중복 목록을 복사했습니다."));
...
function normalizeText(value) {
  return String(value || "").toLowerCase().replace(/공고|안내|모집|사업|신청|지원/g, "").replace(/[^가-힣a-z0-9]/g, "").trim();
}
...
function decorateResourceCards() {
  ...
  $$(".resource-card[data-resource-id]:not([data-radar-enhanced])").forEach((card) => {
    ...
    tools.innerHTML = `<div class="radar-card-facts"><span>${escapeHtml(daysLeftText(item.deadline))}</span><span>${escapeHtml(splitList(item.targets).join(", ") || "대상 확인 필요")}</span><span>${escapeHtml(item.contact || "문의처 확인 필요")}</span><span>${escapeHtml(extractDocuments(item))}</span></div>${missing.length ? `<div class="radar-missing-line">확인 필요: ${missing.map(escapeHtml).join(", ")}</div>` : ""}<div class="radar-card-actions"><button class="ghost-button radar-copy-share" type="button">공유문구 복사</button>${item.contact ? `<button class="ghost-button radar-copy-contact" type="button">연락처 복사</button>` : ""}${item.sourceUrl ? `<a class="ghost-button radar-open-source" href="${escapeHtml(item.sourceUrl)}" target="_blank" rel="noopener">원문 보기</a>` : ""}</div><div class="radar-workflow-box"><div class="radar-progress"><strong>${done}/${CHECKS.length}</strong><span>진행 체크</span></div><div class="radar-check-grid">${CHECKS.map(([key, label]) => `<label><input type="checkbox" data-radar-check="${key}" ${flow.checks?.[key] ? "checked" : ""}> ${escapeHtml(label)}</label>`).join("")}</div><label class="radar-memo-label">우리 기관에서 할 일<textarea class="radar-memo" rows="2" placeholder="예: 김OO 어르신 검토, 사례관리 회의 공유">${escapeHtml(flow.memo || "")}</textarea></label></div>`;
    ...
    tools.querySelectorAll("[data-radar-check]").forEach((input) => input.addEventListener("change", () => saveCheck(item.id, input, tools)));
    tools.querySelector(".radar-memo")?.addEventListener("input", debounce((event) => saveMemo(item.id, event.target.value), 300));
  });
}

function saveCheck(id, input, tools) {
  const workflow = readWorkflow();
  workflow[id] ||= { checks: {}, memo: "" };
  workflow[id].checks ||= {};
  workflow[id].checks[input.dataset.radarCheck] = input.checked;
  writeWorkflow(workflow);
  const count = CHECKS.filter(([key]) => workflow[id].checks[key]).length;
...
function saveMemo(id, memo) {
  const workflow = readWorkflow();
  workflow[id] ||= { checks: {}, memo: "" };
  workflow[id].memo = memo;
  writeWorkflow(workflow);
}

function mountBriefTemplates() {
  const textarea = $("#briefOutput");
  if (!textarea || $("#radarBriefTemplates")) return;
  const panel = document.createElement("section");
  panel.id = "radarBriefTemplates";
  panel.className = "radar-brief-templates";
  panel.innerHTML = `<div class="radar-panel-title"><div><strong>복사 템플릿</strong><span>팀 회의, 사례관리, 카톡 공유, 내부 보고용으로 바로 바꿉니다.</span></div></div><div class="button-row"><button class="ghost-button" data-template="team" type="button">팀 회의용</button><button class="ghost-button" data-template="case" type="button">사례관리용</button><button class="ghost-button" data-template="chat" type="button">카톡 공유용</button><button class="ghost-button" data-template="report" type="button">내부 보고용</button></div>`;
  textarea.insertAdjacentElement("beforebegin", panel);
  panel.querySelectorAll("[data-template]").forEach((button) => button.addEventListener("click", () => {
...
function buildTemplate(type) {
  ...
  if (type === "team") return [`[복지자원 주간 공유 / ${now}]`, `이번 주 확인할 복지자원은 총 ${resources.length}건입니다.`, `마감 임박 자원 ${due.length}건, 고긴급 자원 ${urgent.length}건, 미검토 자원 ${review.length}건입니다.`, "", "우선 검토 자원", ...formatResourceLines([...due, ...urgent].slice(0, 7)), "", "확인 필요: 대상 조건, 신청방법, 문의처, 제출서류"].join("\n");
  if (type === "case") return [`[사례관리 연계 가능 자원 / ${now}]`, `생계 ${byCategory["생계"] || 0}건, 의료 ${byCategory["의료"] || 0}건, 주거 ${byCategory["주거"] || 0}건, 심리정서 ${byCategory["심리정서"] || 0}건이 확인되었습니다.`, "", "대상자 연계 시 먼저 볼 자원", ...formatResourceLines([...resources].sort((a, b) => priorityScore(b) - priorityScore(a)).slice(0, 8)), "", "연계 전 확인: 대상자 동의, 소득·연령 기준, 신청서류, 신청 마감일"].join("\n");
  if (type === "chat") {
    const top = [...resources].sort((a, b) => priorityScore(b) - priorityScore(a))[0];
    return top ? resourceShareText(top) : "[복지자원 공유]\n등록된 자원이 없습니다.";
  }
  return [`[복지자원 수집 결과 보고 / ${now}]`, `1. 신규·등록 자원: ${resources.length}건`, `2. 마감 임박 자원: ${due.length}건`, `3. 고긴급 자원: ${urgent.length}건`, `4. 검토 필요 자원: ${review.length}건`, "", "분야별 현황", ...Object.entries(byCategory).map(([category, count]) => `- ${category}: ${count}건`), "", "우선 조치 필요", ...formatResourceLines([...due, ...urgent].slice(0, 8))].join("\n");
}

function formatResourceLines(items) {
  return items.length ? items.map((item) => `- ${item.title || "제목 미정"} / ${item.agency || "기관 미기재"} / ${item.deadline || "마감 미정"} / ${splitList(item.targets).join(", ") || "대상 확인"}`) : ["- 해당 자원 없음"];
}
...
function focusResourceById(id) {
  ...
  window.setTimeout(() => {
    ...
    const card = $(`.resource-card[data-resource-id="${selectorId}"]`);
...
window.addEventListener("DOMContentLoaded", () => {
  refreshEnhancements();
  window.setInterval(refreshEnhancements, 1200);
});
